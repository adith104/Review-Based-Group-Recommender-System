...
class GDataset(object):
    ...
    def __init__(self, user_path, group_path, num_negatives):
        ...
        self.user_testMatrix = self.load_rating_file_as_matrix(user_path + "Test.txt")
    ...
    def get_user_dataloader(self, batch_size):
        ...
        user_train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, pin_memory=True)
    ...
    def get_group_dataloader(self, batch_size):
        ...
        group_train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, pin_memory=True)
        return group_train_loader

    def get_user_dataloader_test(self, batch_size):
        user, positem_negitem_at_u = self.get_train_instances(self.user_testMatrix)
        test_data = TensorDataset(torch.LongTensor(user), torch.LongTensor(positem_negitem_at_u))
        user_test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=True, pin_memory=True)
        return user_test_loader

    def get_group_dataloader_test(self, batch_size):
        group, positem_negitem_at_g = self.get_train_instances(self.group_testMatrix)
        test_data = TensorDataset(torch.LongTensor(group), torch.LongTensor(positem_negitem_at_g))
        group_test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=True, pin_memory=True)
